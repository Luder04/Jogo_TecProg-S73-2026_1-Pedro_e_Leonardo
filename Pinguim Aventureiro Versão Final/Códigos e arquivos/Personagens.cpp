...
Jogador::Jogador(bool jog2) : Personagem(), jogador2(jog2), pontos(0), forca_pulo(16), forca_andar(5), max_vidas(30), podePular(true), invulneravel(false), tempoInvencivel(0.2f), salvo(false) {
	num_vidas=max_vidas;
	sizex = 25.0f;
	sizey = 55.0f;
    atribuirBotoes();
...
Jogador::Jogador(int pontos, int num_vid, bool jog2, float nx, float ny, float sx, float sy) : Personagem(), jogador2(jog2), pontos(pontos), forca_pulo(16), forca_andar(5), max_vidas(num_vid), podePular(true), tempoInvencivel(0.2f), salvo(false) {
	num_vidas = max_vidas;
	sizex = sx;
	sizey = sy;
	x = nx;
...
//INIMIGO FACIL
Rato::Rato() : Inimigo(), rapidez(0) {
	x = 300+200*(rand()%5)+rand()%60;
	y = 200;

	rapidez=3+2*(rand()%100 + 1)/100.0;
	...
	num_vidas = 1;
	sf::CircleShape* shape = new sf::CircleShape(sizex/2.0);
...
Rato::Rato(float nx, float ny, float sx, float sy) : Inimigo(), rapidez(0) {
	x = nx;
	y = ny;
	rapidez=3+2*(rand()%100 + 1)/100.0;
	velx=rapidez;
...
void Rato::danificar(Jogador* pJog) {
    pJog->operator--(1+nivel_maldade/30);
    if(!direcao){
        pJog->setPosition(pJog->getx() - 15, pJog->gety());
        setPosition(x + 5, y);
    } else {
        pJog->setPosition(pJog->getx() + 15, pJog->gety());
        setPosition(x - 5, y);
...
std::string Rato::getTipo() {
	return "Rato";
}

//INIMIGO MEDIO
Aguia::Aguia() : Inimigo(), tamanho(0), altitude(0) {

    tamanho=10+rand()%6;
	x = 200+rand()%950;
	altitude = 320+rand()%120;
	y=altitude;
	sizex = 6 * tamanho;
	sizey = 3 * tamanho;
...
Aguia::Aguia(float alt, float nx, float ny, float sx, float sy) : Inimigo(), tamanho(0), altitude(alt) {
	tamanho = sx/6;
	x = nx;
	y = ny;
	sizex = sx;
...
void Aguia::executar() {
	...
	if (y>altitude && vely==0){
        vely=-1;
	} else if(y<altitude && vely==0){
        vely=1;
	} else {
        vely=0;
	}
...
Dragao::Dragao(int idn, int vidas, short int nforca, float nx, float ny, float sx, float sy) : Inimigo(), forca(0), atirar(false), timer(0), pProj(NULL) {
	id = idn;
	num_vidas = vidas;
	forca = nforca;
	x = nx;
...
void Dragao::executar() {
    //timer para voltar para a cor normal, se o dragão recebeu dano recentemente
    if(timer==1){
        setTexture("dragao.png");
