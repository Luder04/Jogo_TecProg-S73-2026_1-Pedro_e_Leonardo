...
Menu::Menu() : Ente(), pJog(NULL), submenu_ativo(0), mouse_pressionado(false) {
    ...
    if (!fonte.loadFromFile("BAUHS93.TTF"))
        cout<<"Erro! Nao conseguiu carregar a fonte BAUHS93.TTF."<<endl;
...
void Menu::vitoria(int pontuacao){
    ...
    if(pontuacao>ranking[2]){
        std::string nome;
        cout << "Digite seu nome para o ranking: ";
        cin >> nome;

        if(pontuacao>ranking[1]){
            if(pontuacao>ranking[0]){
                ...
                editarRanking(2,0,pontuacao, nome);
            } else {
                cout<<"Novo top score! Segundo lugar."<<endl;
                editarRanking(2,2,ranking[1],ranking_nome[1]);
                editarRanking(2,1,pontuacao, nome);
            }
        } else {
            cout<<"Novo top score! Terceiro lugar."<<endl;
            editarRanking(2,2,pontuacao, nome);
        }
    }
    submenu_ativo=4;
}
...
void Menu::criarBotao(int id_submenu,const char* nome_botao,std::function<bool()> acao_botao){
    ...
    novo_botao->retangulo.setPosition({ 640.0f, 470.0f - (altura+5.0f)*novo_botao->getId()});//altura+5 porque tem 5 pixels de borda
