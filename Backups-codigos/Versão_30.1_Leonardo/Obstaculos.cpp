...
Plataforma::Plataforma(float nx, float ny, float sx, float sy) : Obstaculo(), altura(sy), largura(sx) {
    qtd_plats++;
...
void Plataforma::flutuar() {
    if(movel){
        if (y < 290) {
            vely += 1.05; //(rand()%10)/100.0; // Experimente outros valores!!!
        }
        else if (y > 200) {
            vely += -1.05;
        }
...
void Lama::setSprite(){
	...
	pSprite->setTextureRect(sf::IntRect({ 0, 0 }, { sizex, altura_sprite }));
}
...
void Lava::setSprite(){
	...
	/*pSprite->setTexture(*pTexture);
    int altura_sprite=sizey*viscosidade*0.5+6;
	pSprite->setPosition(x, y-altura_sprite+2); //desenha a lava um pouco acima do chão
	pSprite->setTextureRect(sf::IntRect({ 0, 0 }, { sizex, altura_sprite }));*/
