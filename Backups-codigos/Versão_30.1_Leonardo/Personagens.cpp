...
Jogador::Jogador(int pontos, bool jog2, float nx, float ny, float sx, float sy) : Personagem(), jogador2(jog2), pontos(pontos), forca_pulo(16), forca_andar(5), max_vidas(30), podePular(true), tempoInvencivel(0.2f), salvo(false) {
	num_vidas = max_vidas;
	sizex = sx;
	sizey = sy;
	x = nx;
...
Aguia::Aguia(float nx, float ny, float sx, float sy) : Inimigo(), tamanho(0) {
	tamanho = sx/6;
	x = nx;
	y = ny;
	sizex = sx;
