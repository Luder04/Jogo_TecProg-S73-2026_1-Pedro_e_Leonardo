...
void GerenciadorColisoes::tocandoChao() {
	if (pJog1->gety() + pJog1->vely < 200) {
		...
		pJog1->vely = 0;
		pJog1->setPosition(pJog1->getx(), 201);
	}
}
	...
	for (i = 0; i < (int)LI.size(); i++) {
		if (LI[i] != NULL) {
			...
			if (proxPos_Y.intersects(boundsInimigo)) {
				float cabeçaInimigo = boundsInimigo.top + boundsInimigo.height;

				if (pJog1->vely <= 0.0f && boundsJogador.top >= cabeçaInimigo) {

					LI[i]->operator--();
					LI.erase(LI.begin() + i);
					i--;

					pJog1->setPosition(pJog1->getx(), cabeçaInimigo);
...
void GerenciadorColisoes::tratarColisoesJogsObstaculo() {
	...
	for (it = LO.begin(); it != LO.end(); it++) {
        ...
        if(pObs != NULL) {
			...
			if (!boundsJogador.intersects(boundsObs)) continue;
			...
			if (boundsJogador.left + boundsJogador.width > boundsObs.left + 5.0f &&
				...
				if (pJog1->vely <= 0.0f) {
					...
					pJog1->setPosition(pJog1->getx(), topoObs);
					pJog1->vely = 0;
					pJog1->podePular = true;
				}
				// De baixo para cima
				else if (pJog1->vely > 0.0f) {
					float fundoObs = boundsObs.top - boundsJogador.height;
					pJog1->setPosition(pJog1->getx(), fundoObs);
					pJog1->vely = -1.0f; // Para a subida e começa a cair
				}
			}
			...
			// Colisão lateral
				...
				if (boundsJogador.left + boundsJogador.width / 2.0f < boundsObs.left + boundsObs.width / 2.0f) {
					pJog1->setPosition(boundsObs.left - boundsJogador.width, pJog1->gety());
				}
				else {
					pJog1->setPosition(boundsObs.left + boundsObs.width, pJog1->gety());
				}
				pJog1->velx = 0.0f;
			}
        }
