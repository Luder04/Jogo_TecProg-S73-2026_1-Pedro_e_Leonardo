...
void Plataforma::flutuacao() {
    if (y < 350) {
        vely += 1 + (rand()%10)/100; // Experimente outros valores!!!
    }
    else if (y > 200) {
        vely -= 1.03;
    }
...
void Lava::setSprite(){
	...
	if (danoso) {
        pSprite->setTextureRect(sf::IntRect({ 0, 0 }, { getSizex(), getSizey() })); //se danoso, pega a parte de cima da imagem de textura (a parte clara)
	}
	else {
	    pSprite->setTextureRect(sf::IntRect({ 0, 16 }, { getSizex(), getSizey() }));//se não danoso, pega a parte de baixo (parte escura)
	}
}
