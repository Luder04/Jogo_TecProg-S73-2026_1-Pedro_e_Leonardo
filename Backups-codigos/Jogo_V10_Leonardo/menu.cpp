...
Menu::Menu() : pJog(NULL), submenu_ativo(0), mouse_pressionado(false) {
}
...
void Menu::criarTitulo(char* nome_titulo){
...
void Menu::criarSubTitulo(int id_submenu,char* nome_subtitulo){
...
void Menu::criarBotao(int id_submenu,char* nome_botao,std::function<bool()> acao_botao){
    Botao *novo_botao = new Botao();
    lista_submenus[id_submenu]->lista_botoes.push_back(novo_botao);
    novo_botao->setNome(nome_botao);
    novo_botao->setId(lista_submenus[id_submenu]->lista_botoes.size());
    novo_botao->acao=acao_botao;

    int altura=70;
    novo_botao->retangulo.setSize(sf::Vector2f(350, altura));
    ...
    novo_botao->retangulo.setPosition({ 640, 425 - (altura+5)*novo_botao->getId()});//altura+5 porque tem 5 pixels de borda
