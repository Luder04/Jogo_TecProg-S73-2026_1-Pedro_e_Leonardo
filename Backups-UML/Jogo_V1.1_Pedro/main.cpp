// Versão 1.1
// Alterações em relação a V1 - Adaptado para o SFML 2.5.1
...
int main()
{
    ...
    if (!font.loadFromFile("BAUHS93.TTF"))
        return EXIT_FAILURE;
    Text titulo("Super Duper Mairo", font);
    ...
    while (window.isOpen())
    {
        ...
        Event event;
        while (window.pollEvent(event)) // O pollEvent realiza "uma ação por vez", enqto o loop principal roda incessantemente*
        {
