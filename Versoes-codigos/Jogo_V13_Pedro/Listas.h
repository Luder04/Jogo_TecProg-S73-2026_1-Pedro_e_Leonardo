...
namespace Listas {
    ...
    public:
            ...
            while (pAux != NULL) {
                ...
                if (pEntidade != NULL) {
                    if (pEntidade->getVidas() > 0 ||  pEntidade->getObs() == true ) {
                        ...
                        std::cout << "Inimigo de id: " << pEntidade->getId() << " morto ignorado no desenho!" << std::endl;
        ...
        void limpar() {
            while (LEs.pPrimeiro != NULL) {
                cout << "Elemento de id: " << LEs.pPrimeiro->getId() << " sendo deletado...";

                LEs.remover(LEs.pPrimeiro);
