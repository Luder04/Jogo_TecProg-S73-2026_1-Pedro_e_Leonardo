...
bool Entidade::getJogador() { return false; }
...
Projetil::Projetil(Dragao* pDrag, float velx, float vely, float nx, float ny, float sx, float sy) : Entidade(), pDragao(pDrag), ativo(true) {
    x = nx;
    y = ny;
    sizex = sx;
    sizey = sy;
